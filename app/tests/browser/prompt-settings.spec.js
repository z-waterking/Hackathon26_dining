...
async function isolate(page, { sourceChanged = false } = {}) {
  const logs = { reads: [], writes: [], unexpected: [], errors: [] };
  const control = { failSave: false, failGet: false };
  let record = {
  ...
  await page.route("**/api/**", async (route) => {
    ...
    if (pathname === "/api/prompt-config" && request.method() === "PUT") {
      ...
      record = { ...record, ...input, sourceChanged: false, version: record.version + 1, baseFingerprint: `base-fixture-${record.version + 1}`, updatedAt: "2026-09-10T03:00:00Z",
        ...
        previews: { ...record.previews, actionGeneration: `服务端保存后的完整指令：${input.actionGenerationText}`, menuSystem: `服务端保存后的全局排菜组合：${input.menuSystemText}` },
      };
      return route.fulfill({ json: record });
    }
  ...
  return { logs, control, saved: () => record };
}
...
test("edits all prompts and paged rules, preserves drafts across navigation, and saves authoritative previews", async ({ page }) => {
  ...
  await expect(page.getByRole("tabpanel", { name: /反馈 → Action/ }).getByText("当前未保存修改尚未进入预览。", { exact: false })).toBeVisible();
  ...
  await page.getByRole("tab", { name: /排菜 System Prompt/ }).click();
  const menuPanel = page.getByRole("tabpanel", { name: /排菜 System Prompt/ });
  ...
  await finalPreviewToggle.click();
  ...
  await expect(menuPanel.getByText("当前未保存修改尚未进入预览。", { exact: false })).toBeVisible();
  ...
  await save.click();
  await expect(page.getByText("与已保存版本一致", { exact: true })).toBeVisible();
  await expect(save).toBeDisabled();
  ...
  await page.getByRole("tab", { name: /排菜 System Prompt/ }).click();
  await expect(finalPreview).toHaveText(`服务端保存后的全局排菜组合：${updatedMenu}`);
  await expect(finalPreview).not.toContainText("服务端寻味列车实际指令");
...
test("changed source rules show original text and can be acknowledged without a local edit", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "保存并生效", exact: true })).toBeEnabled();
  await page.getByRole("tab", { name: /全部排菜规则/ }).click();
...
test("server configuration renders read-only without model calls and remains usable on narrow screens", async ({ page }) => {
  ...
  const menuPanel = page.getByRole("tabpanel", { name: /排菜 System Prompt/ });
  await expect(menuPanel.getByRole("combobox")).toHaveCount(0);
  await menuPanel.locator("summary").filter({ hasText: "最终 Prompt 预览" }).click();
  for (const part of ["排菜指令", "自动加入的规则", "已批准 Action", "输出格式与固定校验"]) {
    await expect(menuPanel.getByText(part, { exact: true })).toBeVisible();
  }
