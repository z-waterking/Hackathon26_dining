...
test("upload old Excel converts into visible ten-column table, persists and imports idempotently", async ({ page }, testInfo) => {
  ...
  await page.getByRole("button", { name: "导入原始文件并转换", exact: true }).click();
  const importDialog = page.getByRole("dialog", { name: "导入原始文件并转换", exact: true });
  await expect(importDialog).toContainText("无需上传目标模板");
  await page.screenshot({ path: testInfo.outputPath("import-original-dialog.png") });
  await page.route("**/api/feedback/upload", (route) => route.fulfill({ status: 503, json: { error: "转换服务暂时不可用，请重试" } }));
  await importDialog.getByLabel("选择原始 Excel 并转换", { exact: true }).setInputFiles(file);
  await expect(importDialog.getByRole("alert")).toHaveText("转换服务暂时不可用，请重试");
  await expect(importDialog.getByLabel("选择原始 Excel 并转换", { exact: true })).toBeEnabled();
  await page.unroute("**/api/feedback/upload");
