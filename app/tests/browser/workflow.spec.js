...
test("feedback reply, aggregate action approval, monthly wordcloud and revision history", async ({ page }, testInfo) => {
  ...
  await expect(page.locator(".action-module")).not.toContainText("示例");
  ...
  await expect(card).toContainText("核验：检查素菜供应");
  expect(generationCalls).toBe(1);
  await page.screenshot({ path: testInfo.outputPath("real-action-list.png") });
  await card.getByRole("button", { name: "查看与审批" }).click();
  const actionDialog = page.getByRole("dialog", { name: "增加清淡素菜", exact: true });
  ...
  await expect(page.getByRole("button", { name: "Prompt 与规则", exact: true })).toBeVisible();
  await page.screenshot({ path: testInfo.outputPath("operations.png"), fullPage: true });
  await actionDialog.getByRole("button", { name: "关闭", exact: true }).click();
  await page.unroute("**/api/actions/summarize");
  await page.route("**/api/actions/summarize", (route) => route.fulfill({ status: 502, json: { error: "Azure AI 请求失败（HTTP 429），服务限流" } }));
  await page.getByRole("button", { name: "AI 生成改善事项", exact: true }).click();
  await expect(page.locator(".action-module").getByRole("alert")).toContainText("已有事项保持不变");
  await expect(card).toContainText("已批准");
...
test("menu workflow shows inspector result, action evidence and trace", async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole("dialog").getByRole("heading", { name: "AI 检验通过 · 待人工审核" })).toBeVisible();
