...
export default function Actions({ data, run, busy, onNavigate }) {
  const [demo, setDemo] = useState(() => !(data.actions || []).some((item) => !item.demo && item.feedbackIds?.length) && (data.actions || []).some((item) => item.demo));
  ...
  useEffect(() => {
    let ignore = false;
    request(`/actions/summary?month=${encodeURIComponent(month)}&demo=${demo}`).then((result) => { if (!ignore) { setSummary({ ...result, month, demo }); setSummaryError(""); } }).catch((error) => { if (!ignore) setSummaryError(error.message); });
    return () => { ignore = true; };
  }, [month, demo, data.actions, data.feedback]);
  const current = summary?.month === month && summary?.demo === demo ? summary : null;
  const loadDemo = () => run(async () => { const result = await request("/demo/actions", {}); setDemo(true); setMonth(""); setStatus(""); return `示例已就绪：${result.feedbackIds.length} 条反馈归纳为 ${result.actionIds.length} 个事项，可直接编辑审批`; });
